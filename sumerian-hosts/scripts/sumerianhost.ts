...
/**
 * This is the script attached to a Sumerian Host, that loads animations 
...
export default class SumerianHost extends Mesh {
  ...
  public async onInitialize(): Promise<any> {
    ...
    const bindPoseOffset = this._scene.animationGroups.find(animGroup => animGroup.name === config.bindPoseOffsetName);
    const animClips = await HostObject.loadCharacterAnimations(this._scene, this, bindPoseOffset, config.animClipPaths);
    ...
    HostObject.addPointOfInterestTracking(host, this._scene, poiConfig, config.lookJoint);
